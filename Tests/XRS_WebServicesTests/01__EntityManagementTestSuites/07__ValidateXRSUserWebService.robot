*** Settings ***
Documentation   Fundamental suite to test XRS User Entity Management Web Services
Library  Collections
...
Suite Setup     Run Keywords  
                ...
                ...  AND  Test Data Setup For XRS User Web Service Test Suite
Suite Teardown  Delete All Sessions
Force Tags      xrsrestwebservicevalidation  xrsuserrestwebservicevalidation

*** Variables ***

*** Test Cases ***
Validate XRS Get User REST Web Services Response ErrorMessage Returns "User '<username>' does not exist."
...
Validate XRS Post User REST Web Services Response Returns Code 201
  [Documentation]  Posts a User and expects a Code value of 201
  ${response} =  Post Users  @{XRS_WEBSERVICE_POST_TEST_USER_LIST}
...
Validate XRS Get User REST Web Services Response Returns 200 OK
...
Validate XRS Put User REST Web Services Response Description Returns "User Edited Successfully."
  [Documentation]  Posts a User and expects a Code value of 201
  ${response} =  Put Users  @{XRS_WEBSERVICE_PUT_TEST_USER_LIST}
...
Validate XRS Get Users REST Web Services Response Returns 200 OK
  ...
  ${wo_slash_response} =  Get Users Response With Forward Slash  &{XRS_WEBSERVICE_USER_TEST_PARAMS}
  ${w_slash_response} =  Get Users Response Without Forward Slash  &{XRS_WEBSERVICE_USER_TEST_PARAMS}
...
Validate XRS Delete User REST Web Services Response Description Returns "User ID does not exist."
...
*** Keywords ***
Test Data Setup For XRS User Web Service Test Suite
  [Documentation]  Keyword for setting up suite variables for User Web Service Tests.
  # Create post test User 1 data.
  &{XRS_WEBSERVICE_POST_TEST_USER_1_DICT} =  Create Dictionary
  ...
  @{XRS_WEBSERVICE_POST_TEST_USER_LIST} =  Create List  ${XRS_WEBSERVICE_POST_TEST_USER_1_DICT}
  Set Suite Variable  @{XRS_WEBSERVICE_POST_TEST_USER_LIST}
  # Create put test User 1 data.
  # Put data must include EncrytpedPassword, but must remain blank ""
  &{XRS_WEBSERVICE_PUT_TEST_USER_1_DICT} =  Create Dictionary
  ...  AccountLocked=${XRS_WEB_SERVICES_TEST_USER.AccountLocked}
  ...  DashboardName=${XRS_WEB_SERVICES_TEST_USER.DashboardName}
  ...  Email=${XRS_WEB_SERVICES_TEST_USER.Email}
  ...  EncryptedPassword=
  ...
  ...  UserName=${XRS_WEB_SERVICES_TEST_USER.UserName}
  ...  UserRole=${XRS_WEB_SERVICES_TEST_USER.UserRole}
  @{XRS_WEBSERVICE_PUT_TEST_USER_LIST} =  Create List  ${XRS_WEBSERVICE_PUT_TEST_USER_1_DICT}
  Set Suite Variable  @{XRS_WEBSERVICE_PUT_TEST_USER_LIST}
  # Create test params
  ${yyyy}	${mm}	${dd} =	Get Time	year,month,day
  &{XRS_WEBSERVICE_USER_TEST_PARAMS} =  Create Dictionary
  ...  OrganizationID=${XRS_GENERAL_INFORMATION.Company.Company_ID}
  ...  IsActive=True
  ...  AsOfDateTime=${mm}/${dd}/${yyyy}
  Set Suite Variable  &{XRS_WEBSERVICE_USER_TEST_PARAMS}
  # Create test params string
  ${XRS_WEBSERVICE_USER_TEST_PARAMS_STRING} =  Catenate  SEPARATOR=&
  ...  OrganizationID=${XRS_WEBSERVICE_USER_TEST_PARAMS.OrganizationID}
  ...  IsActive=${XRS_WEBSERVICE_USER_TEST_PARAMS.IsActive}
  ...  AsOfDateTime=${XRS_WEBSERVICE_USER_TEST_PARAMS.AsOfDateTime}
  Set Suite Variable  ${XRS_WEBSERVICE_USER_TEST_PARAMS_STRING}
