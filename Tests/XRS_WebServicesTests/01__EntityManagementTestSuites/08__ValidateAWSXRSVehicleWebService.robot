*** Settings ***
Documentation   Fundamental suite to test XRS Vehicle Entity Management Web Services
...
Suite Setup     Run Keywords  
                ...
                ...  AND  Test Data Setup For XRS Vehicle Web Service Test Suite
Suite Teardown  Delete All Sessions
Force Tags      xrsrestwebservicevalidation  xrsvehiclerestwebservicevalidation

*** Variables ***

*** Test Cases ***
Validate XRS Get Vehicle REST Web Services Response ErrorMessage Returns "Vehicle identity does not exist."
...
Validate XRS Post Vehicle REST Web Services Response Returns Code 201
  [Documentation]  Posts a Vehicle and expects a Code value of 201
  ${response} =  Post Vehicles  @{XRS_WEBSERVICE_POST_TEST_VEHICLE_LIST}
...
Validate XRS Get Vehicle REST Web Services Response Returns 200 OK
...
Validate XRS Get Vehicle By SID REST Web Services Response Returns 200 OK
...
Validate XRS Put Vehicle REST Web Services Response Description Returns "Vehicle edited successfully."
  [Documentation]  Posts a Vehicle and expects a Code value of 201
  ${response} =  Put Vehicles  @{XRS_WEBSERVICE_PUT_TEST_VEHICLE_LIST}
  &{expected_values} =  Create Dictionary  key=Description  value=Vehicle edited successfully.
  Verify Response List ${response} Has Key ${expected_values.key} And Contains Value ${expected_values.value}

Validate XRS Get Vehicles REST Web Services Response Returns 200 OK
  [Documentation]  Get Vehicles with basic parameters
  ${wo_slash_response} =  Get Vehicles Response With Forward Slash  &{XRS_WEBSERVICE_VEHICLE_TEST_PARAMS}
  ${w_slash_response} =  Get Vehicles Response Without Forward Slash  &{XRS_WEBSERVICE_VEHICLE_TEST_PARAMS}
  Request Should Be Successful  ${wo_slash_response}
  Request Should Be Successful  ${w_slash_response}

Validate XRS Get Vehicles REST Web Services Response Returns 200 OK With Raw String URI
  ...
  ${w_question_response} =  Get Vehicles Raw String URI Response With ? And Parameters ${XRS_WEBSERVICE_VEHICLE_TEST_PARAMS_STRING}
  Request Should Be Successful  ${w_slash_question_response}
  Request Should Be Successful  ${w_question_response}

Validate XRS Delete Vehicle REST Web Services Response Returns 200 OK
...
Validate XRS Delete Vehicle By SID REST Web Services Response ErrorMessage Returns "Vehicle identity '<Vehicle SID>' does not exist."
...
*** Keywords ***
Test Data Setup For XRS Vehicle Web Service Test Suite
  [Documentation]  Keyword for setting up suite variables for Vehicle Web Service Tests.
  # Create post test Vehicle 1 data.
  ${random_vin} =  Get Random Valid VIN From randomvin.com
  &{XRS_WEBSERVICE_POST_TEST_VEHICLE_1_DICT} =  Create Dictionary
  ...
  @{XRS_WEBSERVICE_POST_TEST_VEHICLE_LIST} =  Create List  ${XRS_WEBSERVICE_POST_TEST_VEHICLE_1_DICT}
  Set Suite Variable  @{XRS_WEBSERVICE_POST_TEST_VEHICLE_LIST}
  # Create put test Vehicle 1 data.
  &{XRS_WEBSERVICE_PUT_TEST_VEHICLE_1_DICT} =  Create Dictionary
  ...  OrganizationSID=${XRS_GENERAL_INFORMATION.Company.Company_SID}
  ...  VIN=${random_vin}
  ...  VehicleName=${XRS_WEB_SERVICES_TEST_VEHICLE.VehicleName}
  @{XRS_WEBSERVICE_PUT_TEST_VEHICLE_LIST} =  Create List  ${XRS_WEBSERVICE_PUT_TEST_VEHICLE_1_DICT}
  Set Suite Variable  @{XRS_WEBSERVICE_PUT_TEST_VEHICLE_LIST}
  # Create test params
  ${yyyy}	${mm}	${dd} =	Get Time	year,month,day
  &{XRS_WEBSERVICE_VEHICLE_TEST_PARAMS} =  Create Dictionary
  ...  OrganizationID=${XRS_GENERAL_INFORMATION.Company.Company_ID}
  ...  IsActive=True
  ...  AsOfDateTime=${mm}/${dd}/${yyyy}
  Set Suite Variable  &{XRS_WEBSERVICE_VEHICLE_TEST_PARAMS}
  # Create test params string
  ${XRS_WEBSERVICE_VEHICLE_TEST_PARAMS_STRING} =  Catenate  SEPARATOR=&
  ...  OrganizationID=${XRS_WEBSERVICE_VEHICLE_TEST_PARAMS.OrganizationID}
  ...  IsActive=${XRS_WEBSERVICE_VEHICLE_TEST_PARAMS.IsActive}
  ...  AsOfDateTime=${XRS_WEBSERVICE_VEHICLE_TEST_PARAMS.AsOfDateTime}
  Set Suite Variable  ${XRS_WEBSERVICE_VEHICLE_TEST_PARAMS_STRING}